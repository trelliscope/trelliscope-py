...
from .panel_source import PanelSource, FilePanelSource, LocalWebSocketPanelSource
...
class Panel:
    ...
    def create_panel(df: pd.DataFrame, panel_column: str, panel_options:PanelOptions=None,
        ...
        if is_known_figure_col:
            ...
            panel = ImagePanel(panel_column, source=panel_source, should_copy_to_output=should_copy)

        else:
            raise ValueError(f"Could not determine how to create a panel for {panel_column}")
