...
def facet_panels(df: pd.DataFrame, facet_columns:list, plot_function:types.FunctionType, params:dict) -> pd.DataFrame:
    result_df = df.groupby(facet_columns).apply(lambda mini_df: plot_function(mini_df, **params))
    result_df = result_df.to_frame(name = FACET_PLOT_COLUMN)
    return result_df
